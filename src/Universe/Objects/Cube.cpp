...
Cube::Cube(const std::shared_ptr<DX::DeviceResources>& deviceResources) :
...
winrt::fire_and_forget Cube::CreateDeviceDependentResources()
{
    ...
    auto vertexShaderCode = co_await DX::ReadDataAsync(L"SampleVertexShader.cso");
    auto pixelShaderCode  = co_await DX::ReadDataAsync(L"SamplePixelShader.cso");
    ...
    // Create the pipeline state once the shaders are loaded.
    {
        ...
        state.VS = CD3DX12_SHADER_BYTECODE(vertexShaderCode.data(), vertexShaderCode.Length());
        state.PS = CD3DX12_SHADER_BYTECODE(pixelShaderCode.data(), pixelShaderCode.Length());
...
void Cube::Update(DX::StepTimer const& timer)
{
...
void Cube::LoadState()
{
    ...
    if (state.HasKey(AngleKey))
    {
        m_angle = winrt::unbox_value<float>(state.Lookup(AngleKey));
        state.Remove(AngleKey);
    }
    if (state.HasKey(TrackingKey))
    {
        m_tracking = winrt::unbox_value<bool>(state.Lookup(TrackingKey));
        state.Remove(TrackingKey);
    }
}
...
void Cube::StartTracking()
{
    m_tracking = true;
}

// When tracking, the 3D cube can be rotated around its Y axis by tracking pointer position relative to the output screen width.
void Cube::TrackingUpdate(float positionX)
{
    if (m_tracking)
    {
